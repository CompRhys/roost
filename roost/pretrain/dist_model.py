...
class CrystalGraphPreNet(BaseModelClass):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.node_linear = nn.Linear(2*elem_fea_len, 1)

        self.global_pool = MeanPooling()
        self.global_linear = nn.Linear(elem_fea_len, 1)
    ...
    def forward(self, atom_fea, nbr_fea, self_idx, nbr_idx, mask_idx, cry_idx):
        ...
        atom_nbr_fea = crys_fea[nbr_idx[mask_idx], :]
        atom_self_fea = crys_fea[self_idx[mask_idx], :]

        total_fea = torch.cat([atom_self_fea, atom_nbr_fea], dim=1)

        nodes = self.node_linear(total_fea)
