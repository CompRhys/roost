...
for i, (df, title) in enumerate(zip(df_list, title_list)):
    ...
    if i == 0:
        ...
        ax[j,k].plot(confidence, epi_curve, label="Epistemic Uncertainty")
        ax[j,k].plot(confidence, ale_curve, label="Aleatoric Uncertainty")
        ax[j,k].plot(confidence, res_curve, label="Ground Truth")
        # ax[j,k].set_title(title, pad=20)
    else:
        ax[j,k].plot(confidence, std_curve)
        ax[j,k].plot(confidence, epi_curve)
        ax[j,k].plot(confidence, ale_curve)
