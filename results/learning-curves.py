...
#%%
# Get learning curve lists for SC
sc_rmse_aoe = []
sc_mae_aoe = []
sc_r2_aoe = []

for i in sizes[-6:]:
    df = pd.read_csv("results/ensemble_results_f-5_s-0_t-{}.csv".format(i))
    ...
    rmse = np.sqrt(mse(y_test, y_pred))
    sc_rmse_aoe.append(rmse)
    
    res = mae(y_test, y_pred)
    sc_mae_aoe.append(res)
...
sc_rf = pd.read_csv("results/sc_learn_curve.csv")
sc_size_rf = sc_rf["n_train"]
sc_mae_rf = sc_rf["mae"]
...
plt.subplots_adjust(top=0.99, bottom=0.05, wspace=0.2)

# SuperCon
ax[0].plot(sc_size_rf[-6:], sc_mae_rf[-6:], label="Baseline Model", marker='x', markersize=12)
ax[0].plot(sc_size_rf[-6:].tolist(), sc_mae_aoe, label="Present Work", marker='x', markersize=12)

# ax[0].set_title("MP Non-Metal Bandgaps",  pad=20)
ax[0].set_xscale('log')
ax[0].set_yscale('log')
ax[0].set_yticks((10,8,6,5,4))
ax[0].set_xticks((200,1000,10000))
...
ax[0].set_ylabel("MAE / K", labelpad=10)
ax[0].legend(title=r"$\bf{Data\ Set: SC}$", loc=3, frameon=False)

# Materials Project
ax[1].plot(mp_size_rf, mp_mae_rf, label="Baseline Model", marker='x', markersize=12)
ax[1].plot(mp_size_rf.tolist(), mp_mae_aoe, label="Present Work", marker='x', markersize=12)

# ax[0].set_title("MP Non-Metal Bandgaps",  pad=20)
ax[1].set_xscale('log')
ax[1].set_yscale('log')
ax[1].set_yticks((1.2, 1.1, 1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4))
...
ax[1].legend(title=r"$\bf{Data\ Set: MP}$", loc=3, frameon=False)

# Open Quantum Materials Database
ax[2].plot(oqmd_size_rf, oqmd_mae_rf, label="Baseline Model", marker='x', markersize=12)
ax[2].plot(oqmd_size_rf.tolist(), oqmd_mae_aoe, label="Present Work", marker='x', markersize=12)
# ax[1].plot([oqmd_size_rf.tolist()[-1]], [0.05], linestyle='none', label="ElemNet", marker="x", markersize=12)
ax[2].plot(oqmd_size_elemnet, oqmd_mae_elemnet, label="ElemNet", marker="x", markersize=12)

# ax[1].set_title("OQMD Formation Enthalpy Per Atom",  pad=20)
ax[2].set_xscale('log')
ax[2].set_yscale('log')
ax[2].set_yticks((0.3, 0.2, 0.14, 0.1, 0.07, 0.05, 0.03))
...
ax[2].set_xlabel("Number of Training Points", labelpad=10)
ax[2].set_ylabel("MAE / eV ", labelpad=10)
ax[2].legend(title=r"$\bf{Data\ Set: OQMD}$", loc=3, frameon=False)
